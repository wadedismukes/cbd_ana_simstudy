...
library(treeducken)
library(stringr)
sb_sd_mat <- matrix(nrow = 1, ncol = 2)
sb_sd_mat[,1] <- c(0) # , 1, 2, 3, 2, 3, 3) 
sb_sd_mat[,2] <- c(0) # , 1, 1, 1, 2, 2, 3)
...
for(i in seq_len(length(sims))) {
    ...
    event_history <- treeducken::event_history(sims[[i]])
    ...
    connectivity_graph_print(num_epochs = num_hosts,
                         num_hosts = num_hosts,
                         names_s[i], i)
    write_times(ht[[i]], names_s[i], i)
    write_range_nexus(t(a_mats[[i]]$association_mat), names_s[i], i)
    write.table(event_history$event_history, file = paste0("data/", names_s[i],
						           "/", i, "/",names_s[i], ".csv"))
    ape::write.nexus(st[[i]], file = paste0("data/", names_s[i], "/", i, "/", 
                                            names_s[i],".tre"))
